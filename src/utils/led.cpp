...
void ledTask(void* pvParameters) {
    ...
    for (;;) {
        if (ledRun) {
            if (ringRun == WHITE_RING || ringRun == BLUE_RING ||
                ringRun == YELLOW_RING || ringRun == FAULT_RING) {
                fadeToBlackBy(
                    leds, NUM_LEDS,
                    128);  //Dims the LEDs by 128/256 (1/2) and thus sets the trail's length.
                if (ringRun == WHITE_RING)
                    leds[ringPosition] =
                        CRGB::White;  //setHue: variable to set the LEDs colour
                else if (ringRun == BLUE_RING)
                    leds[ringPosition] = CRGB::Blue;
                else if (ringRun == YELLOW_RING)
                    leds[ringPosition] = CRGB::Yellow;
                else if (ringRun == FAULT_RING)
                    leds[ringPosition] = CRGB::Red;
                ringPosition++;  //Shifts all LEDs one step in the currently active direction

                if (ringPosition == NUM_LEDS)
                    ringPosition =
                        FIRST_LED;  //If one end is reached, reset the position to loop around
                if (ringRun == YELLOW_RING &&
                    (millis() - yellowRingStart) > 10000)
                    ringRun = 0;

                if (ringRun == FAULT_RING)
